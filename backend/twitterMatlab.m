...
searchThing='TAMUHACK OR tamuhack OR ArcAngel';
...
while(c.StatusCode == 'OK')
    ...
    for i = 1:length(temp)
        output(i).time=temp(i).time(5:19);
        output(i).longitude=temp(i).location(1).coordinates(1);
        output(i).latitude=temp(i).location(1).coordinates(2);
    end
    if(~isempty(temp))
